...
from food101n.dataloader import food101n
from food101n.resnet50 import resnet50, wpi, wpi_dec
...
def get_args():
    ...
    parser.add_argument('--epochs', type=int, default=30, help='number of epochs to train (default: 120)')
    parser.add_argument('--gpu_id', type=int, default=0, help='index of gpu to use (default: 0)')

    parser.add_argument('--lr', type=float, default=0.005, help='init learning rate (default: 0.1)')
    parser.add_argument('--seed', type=int, default=0, help='random seed (default: 0)')
    parser.add_argument('--num_classes', type=int, default=101, help='random seed (default: 14)')
    parser.add_argument('--root', type=str, default='your_fold', help='random seed (default: 14)')
...
def adjust_lr(optimizer, epochs):
    lr = args.lr * (0.1 ** int(epochs >= 10)) * (0.1 ** int(epochs >= 20))
...
def train(train_loader,train_meta_loader, model, wpi_, optimizer_model, optimizer_vnet, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(train_loader)):
        ...
        targets_onehot = torch.nn.functional.one_hot(targets, 101).float().cuda()

        v_lambda = wpi_(outputs.detach(), targets, args.sample_number)
        # print((v_lambda * outputs).view(-1, num_classes).size())
        l_f_meta = loss_function((v_lambda * outputs).view(-1, 101),
...
def main():
    ...
    transform_test = transforms.Compose([transforms.CenterCrop((224, 224)),
                                         transforms.ToTensor(),
                                         transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])

    train_dataset = food101n( split='train', transform=transform_train)
    val_dataset = food101n(split='meta', transform=transform_test)  # meta_data
    test_dataset = food101n( split='test', transform=transform_test)
    ...
    wpi_ = wpi(202, 200, 101).cuda()

    optimizer_model = torch.optim.SGD(base_model.params(), lr=args.lr)
    optimizer_vnet = torch.optim.Adam(wpi_.params(), 3e-4)

    best_acc = 0.0
    for epoch in range(args.epochs):
        adjust_lr(optimizer_model, epoch)
        train(train_loader, val_loader, base_model, wpi_, optimizer_model, optimizer_vnet, epoch)
...
if __name__ == '__main__':
    ...
    mytxt = open('food101_log/wpi.txt', mode='a', encoding='utf-8')
    main()
    mytxt.close()
